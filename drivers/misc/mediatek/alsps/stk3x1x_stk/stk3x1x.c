...
static ssize_t stk3x1x_show_nvdata(struct device_driver *ddri, char *buf)
{
	...
	APS_ERR("[#23]offset_H =  %d, offset_L =  %d\n", offset_H,offset_L);
...
static ssize_t stk3x1x_show_GC_20mm(struct device_driver *ddri, char *buf)
{
	...
	APS_ERR("[#23]%s:Start calibrating High threshold...\n", __func__);
	msleep(150);
	reading = stk3x1x_get_ps_reading_AVG(5);

	cci_ps_high_thd = reading; //*****************This is F2N threshold***************
	APS_ERR("[#23]%s:calibrating High threshold done!!!!! cci_ps_high_thd = %d\n", __func__, cci_ps_high_thd);
...
static ssize_t stk3x1x_show_GC_30mm(struct device_driver *ddri, char *buf)
{
	...
	//get ps code for 5 times
	APS_ERR("[#23]%s:Start calibrating Low threshold..\n", __func__);
	msleep(150);
	reading = stk3x1x_get_ps_reading_AVG(5);

	cci_ps_low_thd = reading; //*****************This is N2F threshold***************
	APS_ERR("[#23]%s:calibrating Low threshold done. cci_ps_low_thd = %d\n", __func__, cci_ps_low_thd);
...
static int stk3x1x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk("stk Nina  obj->als_level_num  = %d , obj->als_value_num = %d \n",obj->als_level_num,obj->als_value_num);
...
static int __init stk3x1x_init(void)
{
	...
	printk("stk Nina __init stk3x1x_init\n");
...
static int stk3x1x_local_init(void)
{
	...
	printk("stk Nina2 add i2c_add_driver(&stk3x1x_i2c_driver) ret = %d\n",ret);
	if (ret)
	{	printk("stk delet i2c driver\n");
		...
		{printk("stk Nina i2c_add_driver\n");}
	return 0;
}
static int stk3x1x_local_remove(void)
{
