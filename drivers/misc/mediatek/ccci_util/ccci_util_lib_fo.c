...
// Description: To keep the project id from kernel command parameters
static unsigned int cci_prj_id = 0;
...
/*
	Function: get_cci_prj_id
	Description: Get project id from kernel command line
	0 > VY36
	1 > VY37
	2 > VY38
	4 > VY39
	...
	PS. Please refer cci_hw_id.c
*/
static int __init get_cci_prj_id(char* cmdline)
{
	if(cmdline == NULL || strlen(cmdline) == 0)
	{
		printk("cci prj id is empty\n");
	}
	else
	{
		cci_prj_id = simple_strtoul(cmdline, NULL, 16);
		printk("cci prjject id:%d\n", cci_prj_id);
	}

	return 0;
}
__setup("projectid=", get_cci_prj_id);
...
int ccci_get_fo_setting(char item[], unsigned int *val)
{
	...
	for (i = 0; i < ARRAY_SIZE(ccci_fos_default_setting); i++) {
		...
		if (!strcmp(ccci_name, "MTK_MD1_SUPPORT")) {
			...
			// VY39, VY40
			else if (cci_prj_id == 4 || cci_prj_id == 5)
			{
				ccci_value = 9;	
			} else
			{
				printk("[CEI] ccci_get_fo_setting > Unknown Project ID\n");	
				ccci_value = 9;
			}
